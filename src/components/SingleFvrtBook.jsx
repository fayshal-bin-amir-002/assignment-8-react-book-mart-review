...
const SingleFvrtBook = ({ book, handleDeteleFvrt }) => {
    ...
    return (
        <div className='flex flex-col lg:flex-row gap-4 lg:gap-6 p-4 lg:p-6 border rounded-2xl mb-4 lg:mb-6'>
            <div className='h-[260px] lg:w-[20%] text-center'>
                <img src={image} className='h-full rounded-2xl object-cover inline' />
            </div>
            <div className='lg:w-[80%]'>
                <div className='flex justify-between items-center'>
                    <h3 className='text-xl lg:text-2xl font-bold mb-4'>{bookName}</h3>
                    <span className='text-2xl p-2 bg-red-300 rounded-full text-white cursor-pointer hover:bg-red-500 duration-300' onClick={() => handleDeteleFvrt(book)}><MdDeleteOutline /></span>
                </div>
